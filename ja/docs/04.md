...
グループ領域は、インタラクティブノードおよび各計算ノードで共有されたファイルシステムのディスク領域です。[ABCI利用ポータル](https://portal.abci.ai/user/)から利用管理者権限でグループディスク追加申請を行うことで利用可能になります。追加申請方法については、[ABCI利用ポータル利用手引き](https://docs.abci.ai/portal/ja/)の[ディスク追加申請](https://docs.abci.ai/portal/ja/03/#352)をご参照ください。

グループ領域のパスを確認するには、`show_quota` コマンドを実行してください。コマンドの説明については [ディスククォータの確認](02.md#checking-disk-quota) を参照してください。

#### グループ領域の種類 {#filesystem-type}

グループ領域にはlustreファイルシステム(新領域)とGPFSファイルシステム(旧領域)があり、グループを承認された時期によって利用できる領域が異なります。
2021年度以降に承認されたグループには新領域が割り当てられます。
新領域の参照先は /groups/gAA50NNN (gAA50NNN はグループ名) です。

2020年度までに承認され2021年度にも継続申請したグループ(以下 既存グループ)には従来通り旧領域が割り当てられています。
旧領域の参照先は /groups[1-2]/gAA50NNN ([1-2] は 1 または 2、gAA50NNN はグループ名、以下同様) および /fs3/d00[1-2]/gAA50NNN です。
また、新領域も順次割り当てられ、旧領域の内容をコピーさせていただきます。移行作業は運用側で行います。

#### 2020年度以前から継続利用されているグループのデータ移行 {#areamigration-to-lustre}

既存グループ領域に保存されていたファイルは旧領域に保管されています。当分の間は引き続きこちらのファイルを利用していただくことができます。
...
移行作業が開始されますと、既存グループにも順次新領域が割り当てられていきます。当初の容量は旧領域の2倍です。
データ移行進捗に伴い旧領域のファイルが新領域にコピーされていきますが、データ移行完了までの期間は、利用者様から参照・アクセスすることはできません。参照不可ですが、利用量は増えていきますのでご注意ください。<br/>
移行作業完了後に一定の猶予期間を経て申請されたクォータサイズに戻させていただきます。<br/>
移行作業は 2021年4月から概ね1年かけて運用側で実施させていただきます。作業の順番は /groups2 → /groups1 → /fs3 の予定です。

!!! Note
##### データ移行中について
・運用側がデータ移行するため、利用者から見るとシステム側が自動的にデータ移行を実施しているように見えます。<br/>
・データ移行中に、利用者様は自由に旧領域にR/W可能、ファイルの削除も可能です。<br/>
・データ移行する量を減らすために、旧領域には、可能な限り新規ファイルを置かないでください。不要なファイルは削除してください。可能な範囲で構いません、ご理解お願いいたします。（なお、データ移行完了後は、この制限は無くなります。）
##### データ移行完了のための参照不可期間について
・データ移行完了の確認のために、旧領域を参照不可の期間が数日発生する予定です。<br/>
・その期間、新領域はR/W/削除可能予定です。<br/>
・今までと同じパス/groups[1-2]/gAA50NNNでアクセス可能にするために、運用側がシンボリックリンクを変更します。<br/>
##### データ移行完了後について
・今までと同じパス/groups[1-2]/gAA50NNNでアクセス可能です。<br/>
